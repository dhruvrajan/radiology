...
def kf_diseases(kf_path: str = KF_LABELS_PATH) -> List[str]:
    kf = read_raw_kf_labels(kf_path)
    return list(filter(lambda x: type(x) == str, set(itertools.chain(kf["known_ddx"], kf["ddx1"], kf["ddx2"], kf["ddx3"]))))
