...
from radiology.params import KF_LABELS_PATH, ID_MAP_PATH, NONE, UNK_SYMBOL, COMMON_DISEASES
from radiology.types.reports import Report
...
def transform_location_field(kf: pd.DataFrame):
    ...
    kf["location_cerebral"] = kf["location"].map(lambda x: matches("cerebral", x))
    kf["location_cortical"] = kf["location"].map(lambda x: matches("cortical", x))
    kf["location_cerebellum"] = kf["location"].map(lambda x: matches("cerebellum", x))
    kf["location_deep_gray"] = kf["location"].map(lambda x: "deep_gray" if "deep gray" in x else NONE)
    kf["location_extracerebral"] = kf["location"].map(lambda x: matches("extracerebral", x))
    kf["location_intraventricular"] = kf["location"].map(lambda x: matches("intraventricular", x))
    kf["location_brain_stem"] = kf["location"].map(lambda x: "brain_stem" if "brain stem" in x else NONE)
...
def split_disease(disease):
    ...
    result = "\"" + disease + (" " + "<(" + ", ".join(nicknames) + ")>" if len(nicknames) > 0 else "") + "\""
...
class LabeledReports(Reports):
    ...
    def single_field_dataset(self, field: str):
        return [(report, self.consensus(report.id, field)) for report in self.reports.values()]
